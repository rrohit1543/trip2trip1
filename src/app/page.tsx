...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-slate-900 dark:text-white font-sans antialiased selection:bg-red-600 selection:text-white transition-colors duration-200">
      {/* Navbar Header */}
      <Navbar
      ...
      <main className="pb-20">
        ...
        {activeTab === 'explore' && (
          <div className="space-y-12">
            ...
            <div className="max-w-7xl mx-auto px-4 space-y-6 pt-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-neutral-900 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">
                    Showing verified trips for <strong className="text-slate-900 dark:text-white">{searchDep} &rarr; {searchDest}</strong>
                  </p>
                </div>

                <div className="flex items-center gap-2 text-xs font-bold">
                  <span className="px-3 py-1 bg-slate-100 dark:bg-neutral-950 border border-slate-200 dark:border-neutral-800 rounded-xl text-slate-700 dark:text-neutral-300">
              ...
              <div className="bg-slate-50 dark:bg-black border border-slate-200 dark:border-neutral-900 rounded-3xl p-6 md:p-8 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-xl font-black text-slate-900 dark:text-white">Verified Customer Reviews</h3>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {reviews.map((rev) => (
                    <div key={rev.id} className="bg-white dark:bg-neutral-950 border border-slate-200 dark:border-neutral-900 p-5 rounded-2xl space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-900 dark:text-white text-xs">{rev.customerName}</span>
                      ...
                      <p className="text-xs text-slate-700 dark:text-neutral-300 leading-relaxed">"{rev.comment}"</p>
        ...
        {activeTab === 'live-radar' && (
          <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
            <div className="bg-slate-50 dark:bg-black border border-slate-200 dark:border-neutral-900 p-6 rounded-3xl space-y-4 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="inline-flex items-center gap-2 bg-red-600/20 text-red-600 border border-red-600/40 text-xs font-bold px-3 py-1 rounded-full uppercase mb-2">
                    <Radio className="w-3.5 h-3.5 animate-pulse" /> Live Telemetry
                  </div>
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white">National Live Fleet Radar</h2>
                  <p className="text-xs text-slate-500 dark:text-neutral-400">Real-time GPS coordinates of all group tour buses currently moving across India.</p>
                </div>
              </div>
